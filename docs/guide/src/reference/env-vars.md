...
| `OLLAMA_BASE_URL` | Ollama *(deprecated)* | Optional. Base URL for the Ollama server. Default: `http://localhost:11434`. The `ollama` provider is deprecated ([ADR 0056](../adr/README.md)); prefer the OpenAI provider + `OPENAI_BASE_URL` — see [Local Inference](../providers/local-inference.md). |
