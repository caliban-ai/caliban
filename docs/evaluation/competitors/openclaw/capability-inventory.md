...
>
> **⚠ Category note — read first.** OpenClaw is **not a terminal coding agent**
> in the mould of caliban / Claude Code / Codex. It is a self-hosted,
> multi-channel **personal-assistant gateway** ("a personal AI assistant you
> run on your own devices … it answers you on the channels you already use",
> "the lobster way 🦞"). For *coding* work its `coding-agent` skill **delegates
> to background workers — Codex, Claude Code, or OpenCode** — running in
> isolated git worktrees. So OpenClaw competes with caliban as an
> *orchestration / delivery surface*, and is arguably a **potential consumer of
> caliban** (a candidate worker backend) rather than a like-for-like rival.
> Read the parity matrix with that framing.
>
