...
> counted separately. This matrix has **51** scored rows: **23 ✅ / 18 🟡 /
> 9 🔴 / 1 n/a** (recounted from the file after the 2026-08-16 pass; no rows
> added or deleted, and the one up-tick and one down-tick cancel, so both the
> population and the tally are unchanged from #519).

**Last refreshed:** 2026-08-16 (**caliban-side re-verification pass, #548 /
#557** — no upstream re-baselining; the Grok Build inventory snapshot stays
2026-07-27, #505). Swept every remaining ✅ row for a production call path from
the shipped binary a second time, and re-checked the *evidence* of the
already-down-ticked rows rather than only their ratings. **1 up-tick, 1
down-tick, 8 note-only corrections** across 10 of 51 rows.

The **up-tick** is §C *"Named/resumable headless sessions"*, which was
**understated**: `--session <NAME>`, `-r/--resume`, and `-c/--continue` all ship
and all reach `resolve_resume` on every headless run, where the old note claimed
two of the three were not first-class. Its one true half — no session
list/delete subcommand — survives, and a third claim moved the other way:
`/resume` is a lister, not the "picker" the note called it.

The **down-tick** is §F *"Per-subagent git-worktree isolation"* (**#557**),
reconciling with the [Antigravity matrix](../antigravity/parity-gap-matrix.md),
which reached 🟡 independently on the same code. #519 held this at ✅ because the
background path genuinely reaches `WorktreeManager` — but the tool schema
advertises `isolation: "worktree"` as materializing a worktree *without saying
that `background: true` is also required*, and `background` defaults to `false`.
So the documented call silently gets no isolation, `caliban agents spawn`
hardcodes `isolation_worktree: false`, and `WorktreeOptions` has no consumer
outside its own parse test. Real on one undocumented path, inert on both
documented ones.

The headline **note-only** correction is §E *"Allow/deny/ask rule grammar with
patterns"* (**#548**). Its ✅ survives on grammar, but for a narrower reason than
it claimed, and it was **wrong when written**: #519 wrote the cell at 16:52 on
2026-08-15 citing `Bash(git *)` as a pattern caliban's grammar matched, and #518
/ PR #521 (`f44d726`) only made that true at 18:56 the same day. It is correct
today by accident. Its second claim — `deny > ask > allow` precedence parity —
holds only for the legacy three-bucket flattening, not for the v2 `rules` array,
which is first-match-wins. This is the mirror image of the #519/#522/#523 defect
class: not a row scored optimistically, but a row that was *right about the wrong
thing*, and the lesson it carries is now written into the refresh process below.

§G *"Fast/heavy model split"* picked up the other kind of correction — reachable,
but only behind an opt-in `caliban.toml` `[router]` block, which the old note did
not say (matching the Pi matrix's treatment of the same code). §H's tool-set row
now pins down the `git` entry: caliban has no `Git` tool, git runs through
`Bash`, and Grok's own confirmed tool enum has no `git` either, so that is parity
rather than a gap. Two stale code anchors were re-pointed (§B `dx.rs:151`→`:153`,
§D `compose.rs:1708`→`:1718`, both drifted by line-shifting commits since #519),
and #549 (checkpoint wiring) and #537 (Bedrock/Vertex) are now cited on the rows
that rest on them.
...
| Provider auth / login subcommand | 🟡 | **Note corrected 2026-08-16 (#548 pass); rating unmoved.** There is no `caliban auth`/`login` subcommand at all — `CalibanCommand` (`caliban/src/args.rs:681`) has no such variant — and auth is env vars + `apiKeyHelper`. `/login`, `/logout`, and `/setup-token` are pure stubs that print "lands with the Auth spec" (`caliban/src/tui/slash/model.rs:174`, `:195`, `:216`). **`/status` is not a stub, as the old note claimed** — it prints the active provider's name, then defers the rest: "full provider/auth/subscription status arrives with the Auth spec" (`:148-153`). MCP-side OAuth is separately real (ADR-0023). Grok has OIDC + API-key + device-code (RFC 8628) + external-broker auth |
| Named/resumable headless sessions (`-s/--session-id`, `-r/--resume`, `-c/--continue`) | ✅ | **Up-ticked from 🟡 2026-08-16 (#548 pass) — the row was understated; all three verbs ship and all three are production-wired.** `--session <NAME>` loads *or creates* a named session (`caliban/src/args.rs:379-381`), `-r/--resume <NAME>` resumes one by name (`:253-255`), and `-c/--continue` resumes the most recently updated one (`:239-250`). All three reach `resolve_resume` (`caliban/src/startup/drivers.rs:312-341`) → `resolve_session` (`caliban/src/headless/session_loader.rs:21-51`) on every headless run. The prior note was wrong on two of its three claims: named-session create *is* first-class, and `--continue` is neither missing nor second-class. Two residual narrownesses, neither of them a missing capability: `-c` picks the newest session **store-wide** rather than the newest *in cwd* (`session_loader.rs:35-48` reads `store.list()` unfiltered), and there is no session list/delete **subcommand** — `CalibanCommand` (`caliban/src/args.rs:681`) has no `Sessions` variant. **Third claim corrected downward:** `/resume` is a **lister, not a picker** — it prints sessions newest-first and then admits "full overlay picker (in-place swap) lands in a follow-up" (`caliban/src/tui/slash/session.rs:179-181`), so it cannot swap the live session. That is a TUI limitation and this row scores the *headless* surface |
...
| Allow/deny/ask rule grammar with patterns | ✅ | **Note corrected 2026-08-16 (#548) — the ✅ survives on grammar, but one half of it was accidentally right and the precedence half is overstated.** Grok has `allow`/`deny`/`ask` arrays with `Bash(git *)`/`Read(src**)`/`Edit(**/*.rs)`/`MCPTool(...)` patterns + verbose object form, `deny > ask > allow` (Claude-Code-class). **Grammar — at parity, and now genuinely so.** `split_pattern` normalizes `Tool(<glob>)` before the colon split (`crates/caliban-agent-core/src/permissions_matcher.rs:44-53`; `paren_open` at `:113-119` treats a `:` before the first `(` as the colon form, leaving those parens as literal glob text), so `Bash(git *)` and `Bash:git *` are interchangeable — pinned by tests at `:507` and `:531`, and shared with the `[permissions.auto]` lists so the two config blocks cannot diverge (`crates/caliban-agent-core/src/auto_mode.rs:179-184`). **But this cell predated the behaviour it asserted:** #519 wrote it at 16:52 on 2026-08-15 citing `Bash(git *)` as a pattern caliban's grammar matched, and #518 / PR #521 (`f44d726`) only made that true at 18:56 the same day. It was wrong when written and is correct today by accident — recorded here because the lesson is the reverse of #519's: a *behaviour fix* must sweep the matrices citing the old behaviour. **Precedence — not at parity.** `deny > ask > allow` holds in caliban only as the order used when flattening the **legacy** three-bucket form (`crates/caliban-settings/src/settings.rs:554-571`); the v2 `rules` array is **first**-match-wins, both in the production gate (`PermissionHooks::evaluate_with_rule`, `crates/caliban-agent-core/src/permissions.rs:450-453`) and in the CLI/TUI helper (`evaluate_rules`, `:198-200`), so precedence is positional rather than action-ranked and a Grok `[permission]` block ported rule-for-rule can resolve differently. Grammar parity carries the ✅; ordering semantics are the residual — the same residual that holds the [OpenCode §E *"Per-command bash patterns, last-match-wins"* row](../opencode/parity-gap-matrix.md) at 🟡, where the ordering mismatch is in the row's own label. **Correction retained:** the pre-#519 "finer-grained than Grok's" note was wrong |
...
| Per-subagent git-worktree isolation | 🟡 | **Down-ticked from ✅ 2026-08-16 (#557), reconciling with the [Antigravity matrix](../antigravity/parity-gap-matrix.md) §"Agents", which reached 🟡 on the same code.** #519 kept this ✅ on the grounds that the background path is production-reachable, and it is — `isolation: worktree` → `SpawnSpec.isolation_worktree` (`caliban/src/startup/compose.rs:952-962`) → `crates/caliban-supervisor/src/server.rs:465-479` → `worktree_for_agent` → `WorktreeManager` (ADR-0037). **What that reasoning missed is that the documented call gets nothing.** The tool schema advertises `isolation: "none" \| "worktree"` with the description "`worktree` materializes a dedicated git worktree under .caliban/worktrees/<name> and runs the sub-agent there" (`crates/caliban-tools-builtin/src/agent/agent_tool.rs:201-205`) and **never says that `background: true` is also required**; `background` is a separate key defaulting to `false` (`:69-71`, `:206-209`). On the foreground path the factory contains no reference to `isolation`, `worktree`, or cwd at all (`compose.rs:884-927`), so a model that sets exactly what the schema documents gets a **silent no-op** — the sub-agent runs in the parent's tree believing it is isolated. `caliban agents spawn`, the other user-facing route, hardcodes `isolation_worktree: false` (`caliban/src/agents_cli.rs:474`), so the CLI cannot request one either. And `WorktreeOptions{base_ref, sparse_paths, symlink_directories}` has **no consumer outside its own parse test** (`crates/caliban-tools-builtin/tests/agent_tool.rs:264`): `worktree_for_agent` builds `WorktreeSpec::new(agent_name)` and drops all three (`server.rs:678-684`). Real on one undocumented path, inert on both documented ones — 🟡. Tracked by **#557** |
| Parallel issue-fixing across worktrees | 🟡 | worktree isolation exists — but see the *"Per-subagent git-worktree isolation"* row above, down-ticked 2026-08-16 (#557): it is reachable only via `AgentTool` with **both** `isolation: "worktree"` and `background: true`, and the schema documents only the first. A packaged multi-issue fan-out workflow is 🔴 regardless. Rating unmoved: this row was already 🟡 on the missing fan-out, and the weaker isolation premise does not make it worse |
...
| xAI / Grok provider | 🔴 | **Note corrected 2026-08-15 (#519) — the provider list was wrong.** The binary can construct **four** providers: Anthropic / OpenAI / Ollama / Google (`ProviderKind`, `caliban/src/args.rs:88-95`; `build_provider`, `caliban/src/startup/compose.rs:161-180`). **Bedrock and Vertex are not shipped providers** — `caliban/Cargo.toml` does not depend on `caliban-provider-{bedrock,vertex}`, so no CLI path can construct either (ADR-0034 crates are library-complete but reachable only from their own tests; tracked by **#537**). Still no xAI/Grok backend wired |
| Runtime model swap (`/model`) | ✅ | `/model` runtime swap |
| Fast/heavy model split | ✅ | **Note corrected 2026-08-16 (#548 pass) — reachable, but the old note omitted that it is opt-in.** Purpose-keyed routing + `FastClassifier` (ADR-0022) and router v2 (ADR-0038) are production-wired: `router::try_load` runs on every startup (`caliban/src/main.rs:281`) and `caliban router debug` exposes the resolution table (`caliban/src/subcommands.rs:35`). **But the router only exists when a `caliban.toml` carrying a `[router]` block is discovered** — `try_load` returns `Ok(None)` both when no `caliban.toml` is found and when one is found without `[router]` (`caliban/src/router.rs:41-49`), and the binary then falls back to single-provider construction (`build_provider`, `caliban/src/startup/compose.rs:161-181`). In that default configuration every request goes to one model and the `RequestPurpose::FastClassifier` stamps (`caliban/src/tui/slash/session.rs:348`, `crates/caliban-agent-core/src/auto_mode.rs:395`) route nowhere. Opt-in config is a user-reachable path, not dead machinery, so the ✅ stands — but "ships a fast/heavy split" is true only after the operator writes the config. Matches the [Pi matrix](../pi/parity-gap-matrix.md) §F treatment of the same code (#550) |
...
| Capability (Grok Build) | Caliban | Notes |
|---|---|---|
| read/write/edit/shell(`bash`)/search(`grep`)/`webfetch`/git/subagent | ✅ | **Note corrected 2026-08-16 (#548 pass) — rating unmoved; the `git` entry in the row label needed pinning down.** All of it is registered on every run (`caliban/src/startup/compose.rs:592-616`): `Read`/`Write`/`Edit`/`MultiEdit`/`NotebookEdit`, `Bash` (+`BashOutput`/`KillShell`), `Glob`, `Grep`, `WebFetch`, `WebSearch`, and `AgentTool` (`:1012`). **There is no dedicated `Git` tool** — no `GitTool` anywhere in `caliban/` or `crates/`, and no `git` module under `crates/caliban-tools-builtin/src/`; git runs through `Bash`. That is parity rather than a gap: the inventory's own confirmed Grok tool enum is `bash`, `edit`, `read`, `grep`, `mcp`, `webfetch` ([`capability-inventory.md`](capability-inventory.md) §"Built-in tools"), with **no `git` entry** either — "git workflow operations" is shell-mediated on both sides. `WebSearch` is a caliban extra with no Grok counterpart in the enum |
| Diff-gated edits before apply | 🟡 | **Updated 2026-09-13 (#549): the auto-checkpoint + `/rewind` (revert) half now works** — `CheckpointHook` is wired on the TUI startup path (`build_agent`), the store reaches the `App` from `tui::run`, and `/rewind` restores files (`caliban/src/tui/events.rs::handle_rewind_overlay_key` → `caliban/src/tui/rewind.rs::execute_rewind`). The row **stays 🟡** because *diff-gating before apply* is still absent: no diff library exists in the workspace and the Ask modal shows a truncated `input_summary`, not a diff. _Historical down-tick reason retained for provenance:_ **Down-ticked 2026-08-15 (#519).** Edits *are* gated — `Write`/`Edit` default to `Ask` (`crates/caliban-agent-core/src/permissions.rs:209-232`) and the 4-button Ask modal is real (ADR-0027) — but it is **not diff-gated**: the modal shows a truncated `input_summary` (`caliban/src/tui/ask.rs:42`), and **no diff library exists in the workspace at all** (`similar`/`diffy`/`imara` appear in no `Cargo.toml`). The "auto-checkpoint + `/rewind`" half is worse: `CheckpointHook` is never constructed by the binary and `App::with_checkpoint_store` (`caliban/src/tui/app.rs:573`) carries `#[allow(dead_code, reason = "wired by main.rs once full /rewind action plumbing lands")]` with zero callers, so `checkpoint_store` is always `None` and the overlay renders "(checkpointing not enabled for this session)" (`caliban/src/tui/overlay.rs:826`). **There is no revert.** The `caliban-checkpoint` crate is complete and unit-tested (ADR-0028) — machinery, not a shipped path. **Re-verified 2026-08-16:** still true — `CheckpointHook::new` has construction sites only in `caliban-checkpoint`'s own tests (`src/hook.rs:191`, `tests/plan_mode_marker.rs:23`, `tests/disabled_env.rs:30`) and `with_checkpoint_store` still has zero callers. Six scored rows across six matrices rested on this code with nothing tracking the fix; now tracked by **#549** |
...
1. When a caliban feature lands: edit the relevant row(s) in the same PR,
   ticking 🔴 → 🟡 or 🟡 → ✅.
1a. **When a caliban *behaviour fix* lands, sweep this tree too** — added
   2026-08-16 (#548). A fix that changes how something already-scored behaves
   invalidates every cell describing the old behaviour, and those cells live in
   `docs/evaluation/` as well as `docs/guide/`. #521 swept the guide correctly
   and skipped the matrices, leaving §E asserting a defect that had been fixed
   two hours after the cell was written. This is the *inverse* of the drift the
   scoring rule catches: not a row scored optimistically, but shipped capability
   scored as absent — and a sweep phrased as "re-verify the ✅ rows" walks
   straight past it. Re-read the 🟡/🔴 rows for understatement too.
