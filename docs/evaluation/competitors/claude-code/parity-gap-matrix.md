...
| Subagent in isolated git worktree | 🟡 | **Down-ticked from ✅ 2026-09-09 (#562)**, reconciling with the five sibling matrices ([antigravity](../antigravity/parity-gap-matrix.md) #554/#559, [codex](../codex/parity-gap-matrix.md) #555, [grok-build](../grok-build/parity-gap-matrix.md) #560, [opencode](../opencode/parity-gap-matrix.md) #555/#557, pi), all at 🟡 on the same code. Two corrections. **(1) The old note cited an `isolation: worktree` *frontmatter* route that does not exist** — repo-wide the only `isolation` fields are `SpawnSpec.isolation_worktree` (`crates/caliban-supervisor/src/proto.rs:111`) and the `AgentTool` JSON input `AgentToolInput.isolation` (`crates/caliban-tools-builtin/src/agent/agent_tool.rs:66`); there is no agent-definition frontmatter carrying an `isolation` key, and every production `SpawnSpec` sets `frontmatter_path: None`. **(2) The real mechanism is production-reachable only on the *background* path** (`isolation: worktree` → `SpawnSpec.isolation_worktree`, `caliban/src/startup/compose.rs:959-962` → `crates/caliban-supervisor/src/server.rs:465-479` → `WorktreeManager`, ADR-0037/0052), but the documented `AgentTool` call is a silent no-op: the schema never says `background: true` is also required (`background` defaults to `false`), the foreground factory ignores `isolation` entirely, and `caliban agents spawn` hardcodes `isolation_worktree: false`. Real on one undocumented path, inert on both documented ones. Tracked by **#557** |
