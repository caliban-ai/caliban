...
impl Hooks for PermissionsHook {
    ...
    //
    // This intentionally FIXES a latent bug: the previous hand-written set
    // omitted before_run / after_run / after_run_failure / after_turn_failure,
    // which silently fell through to the trait no-op instead of reaching
    // `inner`. They are now forwarded correctly.
