...
impl Hooks for ModeFilter {
    ...
    // All other 23 events forward verbatim to `inner` (only `before_tool` is
    // overridden above). Generated by the shared decorator macro (#153 AC2);
    // this also closes the same latent gap noted for `PermissionsHook` —
    // before_run/after_run/after_run_failure/after_turn_failure now reach inner.
...
mod tests {
    ...
    #[test]
    fn resolve_startup_mode_cli_overrides_settings() {
        // CLI wins over the settings default (the env value, when set, has been
        // folded into that settings default upstream — #701).
        let m =
    ...
    #[test]
    fn resolve_startup_mode_settings_when_cli_absent() {
    ...
    #[test]
    fn resolve_startup_mode_settings_default_when_cli_absent() {
        let m =
            crate::permission_mode::resolve_startup_mode(None, Some("acceptEdits"), false).unwrap();
        assert_eq!(m, PermissionMode::AcceptEdits);
    }
    ...
    fn resolve_startup_mode_cli_beats_settings_default() {
        let m =
            crate::permission_mode::resolve_startup_mode(Some("plan"), Some("acceptEdits"), false)
                .unwrap();
        assert_eq!(m, PermissionMode::Plan);
    }
